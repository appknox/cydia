...
if [[ "$OSTYPE" == "linux"* ]]; then # Linux usage of repo.me
    ...
    brew list wget &>/dev/null || (echo "Installing wget" && brew install wget)
    brew list xz &>/dev/null || (echo "Installing xz" && brew install xz)
    brew list zstd &>/dev/null || (echo "Installing zstd" && brew install zstd)
    
    if test ! -f "./apt-ftparchive"; then
    ...
    ./apt-ftparchive packages ./debians > Packages 2>/dev/null
    gzip -c9 Packages > Packages.gz
    xz -c9 Packages > Packages.xz
    zstd -c19 Packages > Packages.zst
    bzip2 -c9 Packages > Packages.bz2
    
    ./apt-ftparchive release -c ./assets/repo/repo.conf . > Release 2>/dev/null
